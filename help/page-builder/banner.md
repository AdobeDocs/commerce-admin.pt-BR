---
title: Mídia - Banner
description: Saiba mais sobre o tipo de conteúdo de banner usado para adicionar um componente ilustrado e interativo no [!DNL Page Builder] estágio.
...
Use o _Banner_ tipo de conteúdo para adicionar um componente ilustrado e interativo que envolva os usuários com um plano de ação e um botão no [[!DNL Page Builder] estágio](workspace.md#stage).

>[!NOTE]
>
>O que era anteriormente o _Banner_ no menu Conteúdo, agora é [Bloco dinâmico](../content-design/dynamic-blocks.md).

![Banner em uma página inicial da loja](./assets/pb-banner-homepage.png){width="700" zoomable="yes"}
...
| Configurações | ![Ícone Configurações](./assets/pb-icon-settings.png){width="25"} | Abre a página Editar banner, onde é possível alterar as propriedades do banner e do container. |
...
| Duplicar | ![Ícone Duplicar](./assets/pb-icon-duplicate.png){width="25"} | Faz uma cópia do banner. |
...
1. No [!DNL Page Builder] painel, expandir **[!UICONTROL Media]** e arraste uma **[!UICONTROL Banner]** espaço reservado para o estágio.
   ...
   A variável _[!UICONTROL Upload Image]_e_[!UICONTROL Select from Gallery]_ são incluídos botões para que você possa fazer alterações rápidas no conteúdo do banner diretamente do palco. Também é possível alterar o conteúdo no _[!UICONTROL Edit Banner]_página.

1. Clique no espaço reservado do banner para exibir a [editor de texto](../content-design/editor.md) e insira o conteúdo do banner.
...
1. Passe o mouse sobre o container de banner para exibir a caixa de ferramentas e escolher o _Configurações_ (![Ícone Configurações](./assets/pb-icon-settings.png)) ícone.
...
1. Quando terminar, clique em **[!UICONTROL Save]** no canto superior direito para fechar a _[!UICONTROL Edit Banner]_página.
...
Se você definir a variável _Tipo de plano de fundo_ para `Image`, use as configurações a seguir para definir a exibição da imagem de fundo.
...
- **[!UICONTROL Background Size]** - Defina essa opção para determinar como a imagem de fundo é dimensionada em relação à largura do banner:
...
- **[!UICONTROL Background Position]** - Defina essa opção para determinar como a imagem de fundo é ancorada em relação ao banner:
...
Se você definir a variável _[!UICONTROL Background Type]_para `Video`, use as configurações a seguir para definir a exibição da imagem de fundo.
...
- **[!UICONTROL Infinite Loop]** - Defina como `No` para que o vídeo seja reproduzido uma vez e interrompido. Quando estiver definido como `Yes` (padrão), o vídeo se repete em um loop infinito.

- **[!UICONTROL Lazy Load]** - Defina como `No` para fazer com que o vídeo seja carregado com a página, mesmo quando não estiver visível. Quando estiver definido como `Yes` (padrão), o vídeo é carregado a partir da origem somente quando visível na tela.

- **[!UICONTROL Play Only When Visible]** - Defina como `No` para fazer com que o vídeo comece a ser reproduzido imediatamente depois de ser carregado, independentemente de estar visível. Quando estiver definido como `Yes` (padrão), o vídeo começa a ser reproduzido somente quando está visível.
...
Você pode modificar o conteúdo do banner diretamente no palco ou quando estiver alterando as configurações. As configurações fornecem recursos de conteúdo mais complexos, como links e botões de banner e sobreposições. A posição do conteúdo reflete a [Aparência](#appearance) configuração de posicionamento.
...
1. Passe o mouse sobre o container de banner para exibir a caixa de ferramentas e escolher o _Configurações_ ( ![Ícone Configurações](./assets/pb-icon-settings.png){width="25"} ) ícone.
...
1. Role para baixo até _[!UICONTROL Content]_e use o **[!UICONTROL Message Text]**editor para inserir e formatar o texto do banner.
   ...
   ![Editor de texto da mensagem](./assets/pb-tutorial1-banner-settings-content-message-text.png){width="600" zoomable="yes"}
...
1. Se necessário, especifique um **[!UICONTROL Link]** para o banner.
   ...
   >[!NOTE]
   >
   >A partir da versão 2.4.1, [!DNL Page Builder] O não oferece mais suporte à vinculação do banner e dos links dentro do texto aninhado devido a problemas com a exibição na loja. Se você estiver usando um link na variável _[!UICONTROL Message Text]_, não é possível configurar o_[!UICONTROL Link]_ opção. Se preferir usar um único link para todo o banner, é possível remover todos os links do texto.<br/>
   >
...
1. Se necessário, adicione um botão para solicitar que os clientes sigam o link.
   ...
   - Insira o **[!UICONTROL Button Text]** para aparecer no botão.
...
1. Definir **[!UICONTROL Show Overlay]** a um dos seguintes:
   ...
   Você pode usar uma sobreposição para aplicar uma cor de plano de fundo à área de conteúdo ativa definida pelo [!UICONTROL Appearance] configuração. A imagem de fundo do banner permanece visível por toda a largura do banner.

   Se optar por mostrar uma sobreposição, defina o **[!UICONTROL Overlay Color]**:

   - Clique em **Sem cor** e escolha uma amostra.
   - No **Sem cor** insira um nome de cor válido ou um valor hexadecimal.

   ![Cor da sobreposição](./assets/pb-tutorial1-banner-settings-overlay-color.png){width="600" zoomable="yes"}
...
1. Defina o **[!UICONTROL Border]** estilo aplicado aos quatro lados do contêiner de banner:
...
1. (Opcional) Especifique os nomes dos **[!UICONTROL CSS classes]** na folha de estilos atual para aplicar ao contêiner do banner.

   Separe vários nomes de classe com um espaço.

1. Insira valores, em pixels, para o **[!UICONTROL Margins and Padding]** para especificar as margens externas e o preenchimento interno do banner.
