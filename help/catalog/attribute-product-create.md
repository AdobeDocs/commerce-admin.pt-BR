...
É possível criar atributos ao trabalhar em um produto ou na _[!UICONTROL Product Attributes]_página. As etapas a seguir mostram como criar atributos a partir do_[!UICONTROL Stores]_ menu.
...
1. Para determinar o tipo de controle de entrada usado para entrada de dados, defina **[!UICONTROL Catalog Input Type for Store Owner]** a um dos seguintes:
   ...
   | Data | Exibe um valor de data na variável [formato preferencial](attributes-input-types.md#date-and-time-options) e [fuso horário](../getting-started/store-details.md#locale-options). Os valores de data podem ser selecionados de uma lista ou de um calendário ( ![Ícone de calendário](../assets/icon-calendar.png) ). <br/><br/>**_Nota:_**Dependendo da configuração do seu sistema,_Admin _os usuários podem inserir datas diretamente em um campo ou selecionar uma data no calendário ou na lista. Para obter informações sobre a especificação de valores de data e hora, consulte [Opções de data e hora](attributes-input-types.md#date-and-time-options). |
   | `Yes/No` | Exibe uma lista suspensa com opções predefinidas de `Yes` e `No`. |
   | `Dropdown` | Exibe uma lista suspensa de valores que aceitam apenas uma única seleção. O tipo de entrada Suspensa é um componente essencial da [produtos configuráveis](product-create-configurable.md). |
   ...
   | `Fixed Product Tax` | Permite definir [Taxas de FPT](../stores-purchase/fixed-product-tax.md) com base nos requisitos do seu local. |
   | `Visual Swatch` | Exibe uma amostra que representa a cor, a textura ou o padrão de um produto configurável. A [amostra visual](swatches.md) O pode ser preenchido com um valor de cor hexadecimal ou exibir uma imagem carregada que representa a cor, o material, a textura ou o padrão da opção. |
   | `Text Swatch` | Uma representação baseada em texto de uma opção de produto configurável que é frequentemente usada para o tamanho. [Amostras de texto](swatches.md#text-based-swatches) também pode incluir valores de cor hexadecimais. |
   | `Page Builder` | Um sistema de [Page Builder](../page-builder/introduction.md) espaço de trabalho no local do atributo que facilita a adição de conteúdo envolvente à página do produto. |

   {style="table-layout:auto"}

1. Se você quiser exigir uma seleção de opção antes que o cliente possa comprar o produto, defina **[!UICONTROL Values Required]** para `Yes`.

1. Para [!UICONTROL Dropdown] e [!UICONTROL Multiple Select] tipos de entrada, faça o seguinte:
   ...
   - Clique em **[!UICONTROL Add Option]** e repita a etapa anterior para cada opção que desejar incluir na lista.

   - Selecionar **[!UICONTROL Is Default]** para usar a opção como valor padrão.

   ![Atributo do produto - gerenciar opções](./assets/product-attribute-add-values-colors.png){width="600" zoomable="yes"}
...
1. Insira um único **[!UICONTROL Attribute Code]** em caracteres minúsculos e sem espaços.
   ...
   As opções disponíveis dependem do _[!UICONTROL Catalog Input Type for Store Owner]_configuração.

1. Definir **[!UICONTROL Scope]** para indicar onde no seu [hierarquia de loja](../getting-started/websites-stores-views.md) que o atributo pode ser usado.

1. Se quiser impedir qualquer entrada de valor duplicada, defina **[!UICONTROL Unique Value]** para `Yes`.

1. Para tipos de entrada que são valores inseridos, execute um teste de validade de quaisquer dados inseridos em um campo de texto definindo **[!UICONTROL Input Validation for Store Owner]** ao tipo de dados que o campo deve conter.
...
1. Para adicionar este atributo à variável [Lista de produtos](products-list.md), defina as seguintes opções como `Yes`.

   - **Adicionar às Opções de Coluna** - Inclui o atributo como uma coluna na variável _[!UICONTROL Products]_lista.
   - **Usar em Opções de Filtro** - Adiciona um controle de filtro ao cabeçalho da coluna no _[!UICONTROL Products]_lista.
...
1. Se o atributo deve estar disponível para pesquisa, defina **[!UICONTROL Use in Search]** para `Yes`.

   - Defina o **[!UICONTROL Search Weight]** valor para controlar onde o item aparece nos resultados da pesquisa: 1 (menor peso) a 10 (maior peso).
...
1. Para campos suspensos, de seleção múltipla e de preço, faça o seguinte:
   ...
   - Para usar o atributo como um filtro na navegação em camadas, defina **[!UICONTROL Use in Layered Navigation]** para `Yes`.

   - Para usar o atributo na navegação em camadas nas páginas de resultados da pesquisa, defina **[!UICONTROL Use in Search Results Layered Navigation]** para `Yes`.
...
1. Para permitir que o texto seja formatado com HTML, defina **[!UICONTROL Allow HTML Tags on Frontend]** para `Yes`.
...
1. Após concluir as etapas anteriores, vá para **[!UICONTROL Stores]** > _[!UICONTROL Attributes]_>**[!UICONTROL Attribute Set]**.

1. Selecione o conjunto de atributos necessário na lista e abra-o no modo de edição.

1. Arraste o atributo criado da **[!UICONTROL Unassigned Attributes]** listar para a pasta apropriada no **Grupos** coluna.

1. Quando terminar, clique em **[!UICONTROL Save]**.

## Atributos para produtos configuráveis

Qualquer atributo usado como uma lista suspensa de opções de uma [produto configurável](product-create-configurable.md) deve ter as seguintes propriedades:
...
Antes de excluir um atributo, verifique se ele não está sendo usado por nenhum produto em seu catálogo. Uma maneira fácil de determinar se um atributo está em uso é usar o [Exportar](../systems/data-export.md) ferramenta para verificar a lista de atributos de entidade do produto. Se o atributo não estiver incluído na lista, ele não será usado por nenhum produto no catálogo.
