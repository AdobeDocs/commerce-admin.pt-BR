...
Se [As Cotações B2B estão ativadas](configure-quotes.md) na configuração, a negociação de preços pode ser iniciada por um comprador autorizado de uma empresa ou um representante de vendas.

Os compradores iniciam o processo de negociação de preço por [solicitando uma cotação](quote-request.md) do carrinho de compras. Os representantes de vendas podem iniciar a negociação ao [criando uma cota preliminar para um comprador](sales-rep-initiates-quote.md), atualizando a cotação com os itens do pedido inicial e os preços e enviando-a ao comprador.

Quando a negociação de preço começar, as cotas serão listadas no [Aspas](quotes.md) grade. Todas as negociações entre o comprador e o vendedor ocorrem por email e são iniciadas e rastreadas a partir da exibição detalhada da cotação.
...
Os compradores gerenciam o processo de negociação de cota na loja usando [[!UICONTROL My Quotes]](account-dashboard-my-quotes.md). Enquanto a cota estiver aberta para revisão, seu status na conta do comprador será definido como `Pending`. O comprador pode alterar e ressubmeter a cota mesmo se ela tiver sido recusada ou tiver expirado.

## Etapa 1: exibir a solicitação

1. Na barra lateral Admin, acesse **[!UICONTROL Sales]** > **[!UICONTROL Quotes]**.

   A nova solicitação aparece na variável _[!UICONTROL Quotes]_grade.

1. No _Ações_ clique em **[!UICONTROL View]**.

   ![Nova Cotação](./assets/quote-grid-new.png){width="700" zoomable="yes"}

## Etapa 2: Modificar a cotação

1. Em _[!UICONTROL Quote & Account Information]_, clique no link_ Calendário _(![Ícone de calendário](../assets/icon-calendar.png)) ícone.

   ![Informações de cota e conta](./assets/quote-details-account-information.png){width="575" zoomable="yes"}

1. Escolha um **[!UICONTROL Expiration Date]** para a cotação.

1. Role para baixo até _[!UICONTROL Quote Totals]_e atualize a **[!UICONTROL Negotiated Price]**conforme necessário.
   ...
   ![Aviso de mudança de cotação](./assets/quote-change-notice.png){width="600" zoomable="yes"}

### Adicionar novos produtos à cotação

1. Clique em **[!UICONTROL Add Products by SKU]**.

1. Insira o **[!UICONTROL SKU]** e **[!UICONTROL Qty]** a ser adicionado.
...
Aplicar alterações de item de linha no _[!UICONTROL Items Quoted]_seção, se necessário.

![Aplicar atualizações de item de linha](./assets/quote-apply-line-item-operations.png){width="600" zoomable="yes"}

- Altere o **[!UICONTROL Quantity]** que devem ser comprados pelo preço proposto.

- Selecionar **[!UICONTROL Configure]** e altere as opções do produto.

  A variável [!UICONTROL Configure] opção está disponível somente em um item de linha para um produto configurável

- No **[!UICONTROL Action]** selecione uma ação para atualizar o item:
   - **Item de desconto** para aplicar um desconto como uma porcentagem, quantia fixa ou precificação preferencial.
Opcionalmente, você pode bloquear a quantia de desconto para evitar descontos adicionais. Se o desconto não estiver bloqueado, o desconto do item de linha e qualquer desconto no nível da cotação serão aplicados ao preço do produto.
   - **Deixar um aviso para o comprador** para fornecer ao comprador informações adicionais sobre um item
...
- Para aplicar as alterações e atualizar a cotação para o catálogo compartilhado e as regras de preço, clique em **[!UICONTROL Update Prices]** e clique em **[!UICONTROL Proceed]** para confirmar a atualização.

  ![Itens citados](./assets/quote-detail-items-quoted.png){width="600" zoomable="yes"}

### Atualizar informações de envio

1. Se o comprador incluir uma _Enviar para_ na cotação, clique em **[!UICONTROL Get shipping methods and rates]**.

1. Escolha um método de envio entre as opções disponíveis.

1. Insira um **[!UICONTROL Proposed Shipping Price]**.

   A variável _[!UICONTROL Quote Totals]_são atualizadas para refletir o preço de envio proposto.

### Anexar um documento de suporte

1. No _Adicione seu comentário_ clique em **[!UICONTROL Attach file]**.

   Por padrão, [arquivos anexados](../configuration-reference/sales/quotes.md) pode ter até 2 MB em qualquer um dos seguintes formatos de arquivo: DOC, DOCX, XLS, XLSX, PDF, TXT, JPG ou JPEG, PNG.

1. Escolha o arquivo no diretório.

## Etapa 3: Atualizar informações de nível de cotação e enviar sua resposta

1. No _[!UICONTROL Negotiation]_seção no_[!UICONTROL Comments]_ insira sua resposta na guia **[!UICONTROL Add your comment]** seção.

1. Para incluir um documento de suporte, clique em **[!UICONTROL Attach file]** e selecione o arquivo no diretório.

   O tamanho máximo de arquivo permitido para anexos é de 2 MB.

1. Para aplicar um desconto à cotação como um todo:

   - Em _[!UICONTROL Quote Totals]_no_[!UICONTROL Negotiated Price]_ escolha um dos seguintes tipos de desconto:
...
1. Enviar ou salvar a cotação:
   ...
   - Se a cota estiver pronta para ser enviada de volta ao comprador, clique em **[!UICONTROL Send]**.

   - Para continuar trabalhando na cotação posteriormente, clique em **[!UICONTROL Save as Draft]**.
...
| Botão | Descrição |
|----------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [!UICONTROL Back] | Retorna para a _[!UICONTROL Quotes]_página sem salvar as alterações. |
| [!UICONTROL Print] | Envia a cotação para uma impressora ou a salva como um arquivo PDF. |
| [!UICONTROL Create Copy] | [!BADGE Recursos 1.5.0-beta]{type=Informative url="/help/b2b/release-notes.md" tooltip="Disponível somente para participantes do programa Beta"}`(copy)` anexado ao nome original. Renomeie a nova cotação editando o [!UICONTROL Name] campo. Processe a nova cotação salvando-a como rascunho ou enviando-a ao cliente. |
...
| Campo | Descrição |
|-------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [!UICONTROL Name] | O nome atribuído a uma solicitação de orçamento pelo [comprador](account-company-roles-permissions.md). |
| [!UICONTROL Status] | Indica o estado atual da cotação. O status de uma cotação só pode ser alterado por ação por parte do comprador ou do vendedor. Consulte também a [Configurações de status](quotes.md) do Administrador e da [conta do comprador](account-dashboard-my-quotes.md). |
...
| [!UICONTROL Expiration Date] | Indica o último dia em que a cotação atual é válida. A data de expiração padrão é definida na configuração como 30 dias depois que um comprador envia uma solicitação de cotação. <br/><br/>O vendedor pode substituir a data de expiração padrão inserindo uma data diferente (DD MMM YYYY ) ou escolhendo a data no calendário. A cotação nunca expira se o campo for deixado em branco. <br/><br/>Para cotações abertas, o vendedor recebe uma [notificação por e-mail](../systems/email-templates.md) 48 horas antes do vencimento programado da cotação. Os compradores são notificados 24 horas antes da data de expiração. <br/><br/>O status da cotação muda para _Expirado_ e o comprador não pode fazer mais alterações na cotação. Os preços propostos na cotação revertem para os valores originais do catálogo. <br/><br/>Se uma cotação estiver aberta para revisão pelo vendedor quando a cotação estiver definida para expirar, a data de expiração será redefinida de acordo com o intervalo definido na configuração. <br/><br/>A Data de expiração é o único campo no _Cotação e conta_ seção que pode ser editada durante o processo de revisão. |
| [!UICONTROL Company] | O nome legal do [empresa](account-companies.md) que o comprador representa. |
| [!UICONTROL Company Admin Email] | O endereço de email do [administrador da empresa](account-company-admin.md). |
| [!UICONTROL Sales Rep] | A variável [representante de vendas](account-company-manage.md) que trabalha para o vendedor e é o contato principal atribuído à conta da empresa. |
| [!UICONTROL Shared Catalog (or Customer Group)] | A variável [catálogo compartilhado](catalog-shared.md) ou [grupo de clientes](account-company-customer-group.md) à qual a empresa está atribuída. A cotação pode incluir preços personalizados do catálogo compartilhado atribuído à empresa. |
...
| [!UICONTROL Shipping Method & Price] | O link Obter Métodos de Entrega e Taxas será exibido se o comprador incluir uma _Enviar para_ endereço na cotação. |
...
| Campo | Descrição |
|--------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [!UICONTROL Comments] | A guia Comentários da seção Negociação é usada para informar uma mensagem ao comprador sobre a cota. <br/>**[!UICONTROL Add your comment]**- Os comentários são usados para se comunicar com o comprador durante o processo de negociação. Use os comentários para explicar quaisquer descontos oferecidos na cotação ou o motivo pelo qual uma solicitação de cotação foi recusada.<br/>**[!UICONTROL Attach file]** - O tamanho máximo de arquivo e os tipos de arquivo compatíveis para [arquivos anexados](configure-quotes.md) são determinados pela configuração. Por padrão, um arquivo anexado pode ter até 2 MB e qualquer um dos seguintes tipos de arquivos: DOC, DOCX, XLS, XLSX, PDF, TXT, JPG ou JPEG, PNG. |
...
| [!UICONTROL Catalog Total Price  (Incl./Excl. Tax)] | O preço total dos itens na cotação sem imposto, de acordo com os preços no catálogo compartilhado ou no catálogo principal usado como a base da cotação. Expanda a seção para exibir os valores usados no cálculo, dependendo da [Exibir Subtotal](../configuration-reference/sales/tax.md) na configuração. Opções: <br/>**[!UICONTROL Subtotal (Excl. Tax)]**- O Preço Total do Catálogo sem imposto estimado.<br/>**[!UICONTROL Subtotal (Incl. Tax)]** - O Preço Total do Catálogo sem imposto estimado. <br/>**[!UICONTROL Estimated Tax]**- A quantia de imposto que é estimada para ser aplicada ao Preço Total do Catálogo. |
| Preço negociado | O desconto oferecido ao comprador pode ser baseado em qualquer um dos seguintes itens: <br/>**[!UICONTROL Percentage Discount]**- O desconto como uma porcentagem.<br/>**[!UICONTROL Amount Discount]** - O desconto como um valor fixo. <br/>**[!UICONTROL Proposed Price]**- O preço proposto pelo vendedor.<p>Se todos os itens na cotação tiverem um desconto de item bloqueado, a variável [!UICONTROL Negotiated Price] a seção está desabilitada porque nenhum desconto adicional pode ser aplicado.</p><p>Se um produto tiver um desconto de item de linha que não esteja bloqueado, o desconto de item de linha e o desconto em nível de cotação serão aplicados ao preço do produto.</p> |
| [!UICONTROL Quote Subtotal (Incl./Excl. Tax)] | O preço total proposto de cada item de linha na cotação, com ou sem imposto, dependendo da [cálculo de imposto](../configuration-reference/sales/tax.md) na configuração. |
...
| [!UICONTROL Estimated Tax] | O valor do imposto estimado como devido, conforme especificado na configuração [configurações de exibição](../configuration-reference/sales/tax.md). |
