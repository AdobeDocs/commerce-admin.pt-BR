...
Antes de começar, planeje com antecedência a hierarquia do catálogo de lojas, pois ela é mencionada em toda a configuração. Cada loja pode ter uma [categoria raiz](../catalog/category-root.md), que permite ter um conjunto totalmente diferente de opções do menu principal para cada loja.
...
As instalações de várias lojas do Adobe Commerce devem ser configuradas no Admin e também na linha de comando do servidor. O ADOBE COMMERCE [Guia de configuração](https://experienceleague.adobe.com/docs/commerce-operations/configuration-guide/multi-sites/ms-overview.html) A seção fornece instruções detalhadas para configurar o ambiente do servidor.
...
1. Clique em **[!UICONTROL Create Store]** e defina as opções para o novo armazenamento:
   ...
   - **[!UICONTROL Web Site]** — Escolha um site que seja o pai da nova loja. Se a instalação tiver apenas um site, aceite o padrão (`Main Website`).

   - **[!UICONTROL Name]** — Digite um nome para o novo armazenamento. O nome é somente para referência interna.

   - **[!UICONTROL Code]** — Insira um código em caracteres minúsculos para identificar o armazenamento. Por exemplo: `mainstore`.

   - **[!UICONTROL Root Category]** — Defina como [categoria raiz](../catalog/category-root.md) que define a estrutura de categoria do menu principal da nova loja. Selecione uma categoria raiz específica para o armazenamento, caso já tenha sido criada. Caso contrário, selecione `Default Category`. Você pode voltar mais tarde e atualizar a configuração.
...
1. Clique em **[!UICONTROL Create Store View]** e defina as opções de exibição da loja:
   ...
   - **[!UICONTROL Store]** — Defina para o novo armazenamento que você criou.
   ...
   - **[!UICONTROL Status]** — Defina como `Enabled`.
...
1. Clique em **[!UICONTROL Save Store View]**.
   ...
   ![Nova loja com exibição padrão](./assets/new-store-default-view.png){width="600" zoomable="yes"}

### Etapa 4: configurar o URL de armazenamento

1. No _Admin_ barra lateral, clique **[!UICONTROL Stores]** > _[!UICONTROL Settings]_>**[!UICONTROL Configuration]**.

1. Em _[!UICONTROL General]_no painel esquerdo à esquerda, escolha **[!UICONTROL Web]**.

1. No canto superior esquerdo, defina **[!UICONTROL Store View]** para a exibição criada para o novo armazenamento.

1. Quando solicitado a confirmar [escopo](../getting-started/websites-stores-views.md#scope-settings) alternando, clique em **[!UICONTROL OK]**.

   ![Escolher a exibição da loja](./assets/create-store-config-view.png){width="600" zoomable="yes"}

1. Expandir ![Seletor de expansão](../assets/icon-display-expand.png) o **[!UICONTROL Base URLs]** e insira o URL de base da loja.

   Se necessário, limpe a caixa de seleção **[!UICONTROL Use system value]** para alterar a configuração.

   ![Configuração geral - URLs de base da Web](./assets/config-general-web-base-urls-clear-checkbox.png){width="600" zoomable="yes"}

1. Expandir ![Seletor de expansão](../assets/icon-display-expand.png) o **[!UICONTROL Secure Base URLs]** e repita a etapa anterior se desejar configurar o armazenamento [URL seguro](store-urls.md).
...
1. Defina o **[!UICONTROL Web Site Information]** opções:
   ...
   - **[!UICONTROL Code]** — Insira um código que seja usado no servidor para apontar para o domínio.
   ...
   - **[!UICONTROL Sort Order]** — _(Opcional)_ Informe um número para determinar a sequência na qual esse site está listado com outros sites. Para que este site apareça na parte superior da lista, digite um zero (`0`).

1. Clique em **[!UICONTROL Save Web Site]**.

1. Configurar cada [loja](#add-stores) e [exibição de loja](store-views.md) que é necessário para o novo site.

   Em seguida, é possível abrir o site no modo de edição para definir a loja padrão.

### Etapa 2: configurar o URL de armazenamento

Para configurar o [armazenar URLs](store-urls.md), siga as instruções.
