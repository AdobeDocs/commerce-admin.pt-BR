...
1. Para verificar a precisão dos dados inseridos no campo antes que o registro seja salvo, defina **[!UICONTROL Input Validation]** como o tipo de dados que será permitido no campo. Os valores disponíveis dependem do _[!UICONTROL Input Type]_&#x200B;especificado.
   ...
   >[!NOTE]
   >
   >Para atributos de endereço do cliente definidos pelo sistema, como _Telefone_, _Cidade_ e _Rua_, o Commerce impõe validação interna do lado do servidor independentemente da configuração **[!UICONTROL Input Validation]**. Essas regras padrão limitam os caracteres permitidos para cada campo (por exemplo, números de telefone podem conter apenas dígitos, espaços e determinados símbolos). A configuração **[!UICONTROL Input Validation]** adiciona mais restrições, mas não pode substituir a validação interna, que não pode ser desabilitada por meio da interface do Administrador.
...
1. Para aplicar um filtro de pré-processamento a valores inseridos em um campo de texto, área de texto ou tipo de entrada de várias linhas, defina **[!UICONTROL Input/Output Filter]** como um dos seguintes:
   ...
   - `Strip HTML Tags` - Remove as marcas HTML do texto. Esse filtro pode ajudar a limpar dados que estão colados em um campo de outra fonte que inclui tags do HTML.
...
| [!UICONTROL Grid Filter Condition Type] | Especifica condições de correspondência de filtros para atributos em operações de pesquisa da grade. Isso não afeta o campo _[!UICONTROL Search by keyword]_&#x200B;da grade. Opções: `Partial Match` / `Prefix Match` / `Full Match` |
