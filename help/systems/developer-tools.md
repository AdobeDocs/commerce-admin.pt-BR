...
Sua instância Adobe Commerce ou Magento Open Source pode ser implantada para execução em _produção_ ou _modo de desenvolvedor_. As ferramentas e as definições de configuração projetadas especificamente para desenvolvedores podem ser acessadas somente enquanto a loja está em execução no _modo de desenvolvedor_.
...
Consulte [Compilação MENOS do lado do cliente versus do lado do servidor](https://developer.adobe.com/commerce/frontend-core/guide/css/quickstart/compilation-mode/){:target=&quot;_blank&quot;} na documentação do desenvolvedor do Commerce.

>[!NOTE]
>
>A configuração do fluxo de trabalho de desenvolvimento de front-end está disponível em [Modo de desenvolvedor](../systems/developer-tools.md#operation-modes) somente.
...
1. Expandir ![Seletor de expansão](../assets/icon-display-expand.png) o **[!UICONTROL Front-end Development Workflow]** seção.

1. Definir **[!UICONTROL Workflow Type]** a um dos seguintes:

   - `Client side less compilation` - A compilação ocorre no navegador usando o nativo `less.js` biblioteca.
...
>[!NOTE]
>
>A configuração Configurações do arquivo estático está disponível somente ao trabalhar no [modo de desenvolvedor](../systems/developer-tools.md#operation-modes).
...
Para obter uma lista detalhada das definições de configuração, consulte [_Configurações do arquivo estático_](../configuration-reference/advanced/developer.md) no _Referência de configuração_.
...
>[!NOTE]
>
>Arquivos CSS e JavaScript podem ser otimizados no [Modo de desenvolvedor](../systems/developer-tools.md#operation-modes) somente.
...
**_Para otimizar arquivos de recurso:_**

1. No _Admin_ barra lateral, vá para **[!UICONTROL Stores]** > _[!UICONTROL Settings]_>**[!UICONTROL Configuration]**.

1. No painel esquerdo, expanda **[!UICONTROL Advanced]** e escolha **[!UICONTROL Developer]**.

1. Para otimizar arquivos CSS, expanda ![Seletor de expansão](../assets/icon-display-expand.png) o **[!UICONTROL CSS Settings]** e faça o seguinte:

   - Definir **[!UICONTROL Merge CSS Files]** para `Yes`.
   - Definir **[!UICONTROL Minify CSS Files]** para `Yes`.
...
1. Para otimizar arquivos JavaScript, expanda ![Seletor de expansão](../assets/icon-display-expand.png) o **[!UICONTROL JavaScript Settings]** e faça o seguinte:

   - Definir **[!UICONTROL Merge JavaScript Files]** para `Yes`.
   - Definir **[!UICONTROL Minify JavaScript Files]** para `Yes`.

   ![Configuração avançada - Configurações de JavaScript](../configuration-reference/advanced/assets/developer-javascript-settings.png){width="600" zoomable="yes"}

1. Para minificar arquivos de modelo PHTML, expanda ![Seletor de expansão](../assets/icon-display-expand.png) o **[!UICONTROL Template Settings]** seção e definir **[!UICONTROL Minify Html]** para `Yes`.
...
Antes de usar uma ferramenta como [dicas do caminho do modelo](#template-path-hints), adicione seu endereço IP à inclui na lista de permissões Restrições de cliente do desenvolvedor para evitar interromper a experiência de compra dos clientes na loja. Se você não souber seu endereço IP, poderá pesquisá-lo online.

>[!NOTE]
>
>É possível definir Restrições de Cliente de Desenvolvedor em [Modo de desenvolvedor](../systems/developer-tools.md#operation-modes) somente.

Para obter informações técnicas, consulte [VCL personalizado para permitir solicitações](https://experienceleague.adobe.com/docs/commerce-cloud-service/user-guide/cdn/custom-vcl-snippets/fastly-vcl-allowlist.html) no _Guia do Commerce na infraestrutura em nuvem_.

**_Para adicionar seu endereço IP ao arquivo de inclui na lista de permissões:_**

1. No _Admin_ barra lateral, vá para **[!UICONTROL Stores]** > _[!UICONTROL Settings]_>**[!UICONTROL Configuration]**.

1. No painel esquerdo, expanda **[!UICONTROL Advanced]** e escolha **[!UICONTROL Developer]**.

1. Expandir ![Seletor de expansão](../assets/icon-display-expand.png) o **[!UICONTROL Developer Client Restrictions]** seção.

   ![Configuração avançada - restrições do cliente do desenvolvedor](../configuration-reference/advanced/assets/developer-developer-client-restrictions.png){width="600" zoomable="yes"}
...
>[!NOTE]
>
>As Dicas de caminho do modelo podem ser editadas em [modo de desenvolvedor](../systems/developer-tools.md#operation-modes) somente.
...
![Exemplo da loja - dicas de caminho do modelo](./assets/storefront-template-path-hints.png){width="700" zoomable="yes"}

### Etapa 1: adicionar o endereço IP ao arquivo de inclui na lista de permissões

Antes de usar as dicas de caminho de modelo, adicione seu endereço IP à [➡ incluir na lista de permissões](#client-restrictions) para evitar interferência com clientes que estão fazendo compras na loja. Quando terminar, limpe o cache do Commerce para remover todas as dicas da loja.
...
1. Expandir ![Seletor de expansão](../assets/icon-display-expand.png) o **[!UICONTROL Debug]** e faça o seguinte:

   ![Configuração avançada - depurar](../configuration-reference/advanced/assets/developer-debug.png){width="600" zoomable="yes"}

   - Para ativar dicas de caminho de modelo para o armazenamento, defina **[!UICONTROL Enabled Template Path Hints for Storefront]** para `Yes`.

   - Para ativar dicas de caminho de modelo para o armazenamento somente quando o URL inclui a variável `templatehints` parâmetro, definir **Habilitar dicas para vitrine eletrônica com parâmetro de URL** para `Yes`. Em seguida, defina o valor do parâmetro, se necessário. O valor padrão é `magento`, mas você pode usar um valor personalizado. Por exemplo, se você alterar o valor para `lorem`, você usaria `mymagento.com?templatehints=lorem` para exibir dicas de template.

   - Para ativar dicas de caminho de modelo para o Administrador, defina **[!UICONTROL Enabled Template Path Hints for Admin]** para `Yes`.

   - Para incluir os nomes dos blocos, defina **[!UICONTROL Add Block Class Type to Hints]** para `Yes`.

1. Quando terminar, clique em **[!UICONTROL Save Config]**.

### Etapa 3: limpar o cache

1. No _Admin_ barra lateral, vá para **[!UICONTROL System]** > _[!UICONTROL Tools]_>**[!UICONTROL Cache Management]**.

1. No canto superior direito, clique em **[!UICONTROL Flush Magento Cache]**.

## Converter em linha

Você pode usar a ferramenta Traduzir em linha no [modo de desenvolvedor](../systems/developer-tools.md#operation-modes) para retocar o texto na interface e refletir sua voz e marca. Quando o modo Traduzir em linha está ativado, qualquer texto na página que pode ser editado é contornado em vermelho. É fácil editar rótulos de campo, mensagens e outros textos que são exibidos na loja e no Administrador. Por exemplo, muitos temas usam terminologia como _Minha conta_, _Minha lista de desejos_, e _Meu painel_, para ajudar os clientes a encontrar uma solução. No entanto, você pode preferir simplesmente usar as palavras _Conta_, _Lista de desejos_, e _Painel_.
...
Se a loja estiver disponível em vários idiomas, você poderá fazer ajustes no texto traduzido para a localidade. No servidor, o texto da interface é mantido em um arquivo CSV separado para cada bloco de saída e é organizado por localidade. Como uma abordagem alternativa, em vez de usar o _Traduzir em linha_ também é possível editar os arquivos CSV diretamente no servidor. Os arquivos de tradução são armazenados no `app/code/Magento/<module_name>/i18n/<language_locale>.csv`.
...
1. Defina o **[!UICONTROL Actions]** controle para `Disable` e clique em **[!UICONTROL Submit]**.

### Etapa 2: ativar a ferramenta Traduzir em linha

1. No _Admin_ barra lateral, vá para **[!UICONTROL Stores]** > _[!UICONTROL Settings]_>**[!UICONTROL Configuration]**.

1. Para trabalhar com uma visualização de loja específica, defina a **[!UICONTROL Store View]** a ser atualizado.

1. No painel esquerdo, expanda **[!UICONTROL Advanced]** e escolha **[!UICONTROL Developer]**.

1. Expandir ![Seletor de expansão](../assets/icon-display-expand.png) o **[!UICONTROL Translate Inline]** seção.

   Limpe a **[!UICONTROL Use Website]** conforme necessário para modificar essas configurações.

   A variável _[!UICONTROL Enabled for Admin]_a opção não está disponível ao editar uma exibição de loja específica.
...
1. Definir **[!UICONTROL Enabled for Storefront]** para `Yes`.
...
1. Abra a vitrine em um navegador e vá para a página que deseja editar.
   ...
   Se necessário, use o seletor de idioma para alterar a exibição da loja. Cada cadeia de texto que pode ser traduzida é contornada em vermelho. Quando você passa o mouse sobre qualquer caixa de texto, um ícone de livro ( ![Ícone de livro](../assets/icon-book.png) ) será exibida.

1. Clique no ícone do livro para abrir a _Traduza_ e faça o seguinte:

   - Se a alteração for para a exibição de loja específica, selecione a **[!UICONTROL Store View Specific]** caixa de seleção

   - Insira o novo **[!UICONTROL Custom]** texto.
...
1. Definir **[!UICONTROL Store View]** à exibição específica que foi editada.

1. No painel esquerdo, expanda **[!UICONTROL Advanced]** e escolha **[!UICONTROL Developer]**.

1. Expandir ![Seletor de expansão](../assets/icon-display-expand.png) o **[!UICONTROL Translate Inline]** seção.

1. Definir **[!UICONTROL Enabled for Frontend]** para `No`.
...
1. Defina o **[!UICONTROL Actions]** controle para `Enable` e clique em **[!UICONTROL Submit]**.
