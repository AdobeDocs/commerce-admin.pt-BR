...
1. Insira um número para estabelecer o **[!UICONTROL Priority]** desta regra em relação a outras regras.
   ...
   A configuração **[!UICONTROL Priority]** determina qual regra se aplica quando um produto atende às condições de várias regras de preço. A regra com a prioridade mais alta (número mais baixo, por exemplo, 0, 1, 2, 3...) entra em vigor.
...
- Se você adicionar a condição de atributo de produto `[!UICONTROL Category]` a produtos agrupados, a regra de preço será aplicada corretamente somente se todos os itens secundários compartilharem a mesma categoria. Se os itens filho não estiverem na mesma categoria, use uma promoção [Regra de Preço do Carrinho](price-rules-cart-create.md).&quot;
...
1. Para interromper o processamento de outras regras depois que esta regra for aplicada, defina **[!UICONTROL Discard Subsequent Rules]** como `Yes`.
   ...
   Configurar esse valor como `Yes` é uma proteção para impedir que o sistema aplique vários descontos (regras) ao mesmo produto.
...
1. Atualizar propriedades de uma regra:
   ...
   - ![Magento Open Source](../assets/open-source.svg) (somente Magento Open Source) Clique na regra na lista para exibir a página _[!UICONTROL Rule Information]_.
