...
description: Use o [!UICONTROL Customers] grade para localizar qualquer conta de cliente e acessar informações de contas de clientes individuais.
...
Use o _[!UICONTROL Customers]_grade para localizar qualquer conta de cliente. Você pode usar o padrão [controles do local de trabalho](../getting-started/admin-workspace.md) para filtrar a lista, altere o [layout de coluna](../getting-started/admin-grid-controls.md), salvar visualizações e exportar dados. A variável [Controle de ações](../getting-started/admin-actions-control.md) acima da grade pode ser usada para aplicar uma operação a vários registros de clientes.

![Todos os Clientes](assets/customers-all-customers.png){width="700" zoomable="yes"}

Consulte [Atualizar perfil de cliente](update-account.md) para obter informações sobre como fazer atualizações manuais em uma conta do cliente.
...
Em configurações de várias lojas e vários sites com um [escopo da conta do cliente](../customers/customer-account-scope.md), uma conta de cliente pode assinar boletins informativos em vários sites ou lojas. Se você aplicar a variável _Assinar_ ação para uma conta de cliente, ativa a assinatura do boletim informativo somente para a exibição padrão de site/loja.

* Defina o **[!UICONTROL Actions]** controle para `Subscribe to newsletter`.
...
Em configurações de várias lojas e vários sites com um [escopo da conta do cliente](customer-account-scope.md), uma conta de cliente pode assinar boletins informativos para vários sites/lojas. Se você aplicar a variável _Cancelar inscrição_ ação para uma conta de cliente, todas as assinaturas ativas são canceladas.

1. Defina o **[!UICONTROL Actions]** controle para `Unsubscribe to newsletter`.

1. Quando for solicitada a confirmação, clique em **OK**.

### Atribuir um grupo de clientes

1. Defina o **[!UICONTROL Actions]** controle para `Assign a customer group`.
...
1. No menu Cabeçalho da tabela, clique em **[!UICONTROL Export]** e selecione o formato desejado:
