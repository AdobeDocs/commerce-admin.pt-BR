...
Todas as ordens têm um status de ordem associado a um estágio no processamento da ordem [fluxo de trabalho](order-processing.md). O status de cada pedido é mostrado no _Status_ coluna da _Pedidos_ grade. Sua loja tem um conjunto de configurações predefinidas de status de pedido e estado de pedido. O estado do pedido descreve a posição de um pedido no fluxo de trabalho.

![Status do pedido](./assets/stores-order-status-column.png){width="700" zoomable="yes"}

>[!TIP]
>
>Uma ordem parcialmente reembolsada permanece em `Processing` status até **_all_** os itens solicitados (incluindo itens reembolsados) são entregues. O status do pedido não é alterado para `Complete` quando até mesmo um item da ordem ainda não foi entregue.

## Fluxo de trabalho do status do pedido

![Fluxo de trabalho do status do pedido](./assets/order-workflow.png)

## Status predefinido

| Status do pedido | Código de status |  |
|--- |--- |--- |
| Processamento | `processing` | Quando o estado dos novos pedidos é definido como &#39;Processando&#39;, a variável _Faturar Todos os Itens Automaticamente_ fica disponível na configuração. As faturas não são criadas automaticamente para pedidos feitos usando o Cartão Presente, o Crédito da Loja, os Pontos de Recompensa ou outros métodos de pagamento offline. |
| Suspeita de fraude | `fraud` | Às vezes, os pedidos pagos pelo PayPal ou outro gateway de pagamento são marcados como _Suspeita de fraude_. Esse status significa que o pedido não tem fatura emitida e o email de confirmação também não é enviado. |
...
| Abertura | `STATE_OPEN` | Esse status significa que um pedido ou memorando de crédito ainda está aberto e pode exigir outras ações. |
...
Além das configurações de status de pedido predefinidas, você pode criar suas próprias configurações de status de pedido personalizadas, atribuí-las a estados de pedido e definir um status de pedido padrão para estados de pedido. O estado do pedido indica a posição do pedido no fluxo de trabalho de processamento do pedido e o status do pedido define o estado do pedido. Por exemplo, você pode precisar de um status de pedido personalizado, como `packaging"`, `backordered`ou um status específico para suas necessidades. Você pode criar um nome descritivo para o status personalizado e atribuí-lo ao estado de pedido associado no fluxo de trabalho.
